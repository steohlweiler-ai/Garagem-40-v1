...
const App: React.FC = () => {
    ...
    // Wrapper around loadServices to automatically retry on timeout/failure
    const loadServicesWithRetry = async (reset: boolean = false, maxRetries: number = 2) => {
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                console.log(`🔄 [RETRY] Attempt ${attempt + 1}/${maxRetries + 1} to load services...`);
                await loadServices(reset);
                console.log(`✅ [RETRY] Successfully loaded services on attempt ${attempt + 1}`);
                return; // Success!
            } catch (err: any) {
                ...
                if (isTimeout && canRetry) {
                    ...
                    console.warn(`⚠️ [RETRY] Attempt ${attempt + 1} failed (timeout), retrying in ${waitTime}ms...`);
                    await new Promise(resolve => setTimeout(resolve, waitTime));
                    continue;
                }

                // Final failure or non-retryable error
                console.error(`❌ [RETRY] All ${maxRetries + 1} attempts failed or non-retryable error`);
    ...
    const loadServices = async (reset: boolean = false) => {
        ...
        // OPTIMIZED: 45s for first load (cold start), 20s for subsequent loads
        // Chrome needs more time on cold start due to cache initialization
        const timeoutDuration = isInitialLoad ? 45000 : 20000;
        const timeout = setTimeout(() => {
            console.error(`⏱️ [ERROR] Timeout ao buscar serviços após ${timeoutDuration}ms — abortando`);
            controller.abort();
        }, timeoutDuration);
    ...
    const handleSmartRetry = () => {
        ...
        loadServicesWithRetry(true).catch(() => {
    ...
    const refreshServices = async () => {
        await Promise.all([loadStats(), loadServicesWithRetry(true)]);
    };
    ...
    useEffect(() => {
        ...
        if (storedVersion !== CURRENT_APP_VERSION) {
            console.warn('⚠️ [CACHE] Version mismatch detected. Will initiate NUCLEAR RESET after 3s delay...');

            // DELAYED RESET: Wait 3 seconds to allow initial data load before resetting
            // This prevents interrupting legitimate first loads
            setTimeout(() => {
                console.warn('⚠️ [CACHE] Initiating NUCLEAR RESET after delay...');
                performNuclearReset();
            }, 3000);
    ...
    // Nuclear Reset function (called after delay if version mismatch)
...
// ===================== CONSOLIDATED & OPTIMIZED DATA LOADING =====================
useEffect(() => {
...
// Load vehicles and clients for filtering (OPTIMIZED - only once on auth)
useEffect(() => {
...
}, [isAuthenticated]); // Removed 'services' dependency to prevent excessive reloading
...
const processedServices = useMemo(() => {
    let filtered = services.filter(s => {
        ...
        const search = normalize(debouncedSearch); // OPTIMIZED: using debounced search
...
}, [services, debouncedSearch, dashboardFilter, dashboardAdvancedFilters, delayCriteria, allVehicles, allClients]);
