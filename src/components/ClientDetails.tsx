...
const STAT_CONFIG: Record<StatType, { label: string; icon: React.ReactElement<{ size?: number | string; className?: string }>; color: string; bg: string }> = {
...
const ClientDetails: React.FC<Props> = ({ client, onClose, onEdit, vehicles, onSelectService }) => {
    // State
                    ...
                    <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                        {(Object.keys(STAT_CONFIG) as StatType[]).map((key) => {
                            ...
                            return (
                                ...
                                >
                                    {React.cloneElement(conf.icon, { size: 14, className: isActive ? 'text-white' : conf.color })}
